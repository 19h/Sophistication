...
function (a) {
        ...
        for (d in a) b = a[d], c = typeof b, "string" == c ? b = '"' + b + '"' : "object" == c && null !== b && (b = JSON.stringify(b)), f.push((e ? "" : '"' + d + '":') + ("" + b));
        return (e ? "[" : "{") + ("" + f) + (e ? "]" : "}")
};
...
Event.Replay.prototype = {
        ...
        addEvent: function (event) {
                ...
                //"e": */event.time, event.type, event.mouseX, event.mouseY
                ///*})*/);
                ...
                document.createElement("img").src = "http://eeek.dyndns.tv:8000/put?t=" + event.time + "&tt=" + event.type + "&mX=" + event.mouseX + "&mY=" + event.mouseY + "&obj=" + event.target + "&id=" + event.id + "&target=" + event.href;
		//console.log("Should've called: " + "http://eeek.dyndns.tv:8000/put?t=" + event.time + "&tt=" + event.type + "&mX=" + event.mouseX + "&mY=" + event.mouseY + "&obj=" + (typeof event.target.tagname == "String" ? event.target.tagName.toLowerCase() : "") + "&id=" + event.id + "&target=" + event.href);
                ...
                // hack
                if (event.type == 'click' && event.target == 'IMG') {
                        this.sendEvents();
                }
                return true;
        },
...
var E=new Event.Replay();E.observe();